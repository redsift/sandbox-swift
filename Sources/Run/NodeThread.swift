...
class NodeThread {
  ...
  public init(name: Int, addr: String){
    ...
    do {
        ...
        let _ = endPoint // only to keep compiler happy
        ...
        while (true) {
            _ = try self.socket!.receiveMessage(receiveMode: .Blocking, sendMode: .Blocking) { received in
                ...
                guard let computeF = Sift.computes[self.threadName] else {
                    putStdOut("Index (\(self.threadName)) not available in Sift.computes: \(Sift.computes)\n")
                    exit(1)
                }
